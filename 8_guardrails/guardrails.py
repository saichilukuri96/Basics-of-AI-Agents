...
def get_customer_info_tool(customer_name:str) -> str:
    ...
    """
    customers = {
        "Krishna" : {"email": "krishna_001@abc.com",
...
agent = create_agent(
    ...
    middleware= [PIIMiddleware("credit_card",strategy = "mask",apply_to_tool_results = True),PIIMiddleware("email",strategy = "redact",apply_to_tool_results =True,apply_to_output=True),]
)

result = agent.invoke({"messages" :
                       [{"role":"user","content":"Give me information about customer Krishna"}]})
